...
TIP: Para las actualizaciones de sitios oscuros, puede reducir el tiempo de carga si el paquete de firmware de computación y el nodo de gestión son locales.
...
La interfaz de usuario muestra un mensaje una vez completada la actualización. Es posible descargar registros una vez completada la actualización.



| El nodo de gestión se encuentra en un sitio oscuro sin conectividad externa.  a| 
.. Seleccione el clúster que desea actualizar.
.. Seleccione *examinar* para cargar el paquete de firmware de computación que descargó dehttps://mysupport.netapp.com/site/products/all/details/netapp-hci/downloads-tab["Sitio de soporte de NetApp"^].
...
| No se puede detectar | El control del cloud híbrido de NetApp no cuenta con conectividad externa para llegar al repositorio de software en línea. Este estado también se muestra si el control de cloud híbrido de NetApp no puede consultar el nodo de computación cuando el activo del nodo de computación no tiene la etiqueta de hardware. 
...
+
... Abra la interfaz de usuario de la API DE REST de un paquete en el nodo de gestión:
+
[listing]
----
...
.. Busque el ID de bundle de firmware de computación:
+
... En la interfaz de usuario DE LA API DE REST, seleccione *GET /packages*.
... Seleccione *probar*.
... Seleccione *Ejecutar*.
... En la respuesta, copie y guarde el nombre de paquete de firmware de computación (`"packageName"`) y la versión (`"packageVersion"`) para usar en un paso posterior.
